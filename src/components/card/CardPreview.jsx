...
// This component can now accept an optional `initialData` prop.
// If no prop is passed, it falls back to using the global context.
const CardPreview = forwardRef(({ initialData }, ref) => {
  const [isFlipped, setIsFlipped] = useState(false);
  const globalContext = useCardContext();

  // Use initialData if provided, otherwise use data from the context
  ...
  const currentPalette = palettes[cardData.colorPalette] || palettes.warm;
  const currentFont = fonts[cardData.fontFamily] || fonts.poppins;
  ...
  const StaticCard = ({ isFront }) => (
    <div 
        className="w-[400px] h-[533px] rounded-2xl overflow-hidden flex flex-col justify-center items-center p-8 text-center"
        style={cardStyle}
    >
        {isFront ? (
            <>
  ...
  return (
    <div className="w-full max-w-md mx-auto aspect-[3/4] p-4">
        {/* Hidden container for html2canvas to capture */}
        <div ref={ref} style={{ position: 'fixed', left: '-200vw', top: 0 }}>
        ...
        <div className="w-full h-full" style={{ perspective: '1200px' }}>
          <div 
            ...
            onClick={flipCard}
          >
            <div className="absolute w-full h-full rounded-2xl shadow-lg" style={{ backfaceVisibility: 'hidden' }}>
              <StaticCard isFront={true} />
            </div>
            <div className="absolute w-full h-full rounded-2xl shadow-lg" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
              <StaticCard isFront={false} />
            </div>
          </div>
        </div>
