...
const FrameSelector = ({ frames, selectedFrame, onSelect }) => {
  return (
    <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Image Frame</label>
        ...
        {Object.entries(frames).map(([key, frame]) => (
            ...
            <p className="text-xs font-semibold text-center text-primary capitalize">{frame.name}</p>
            </button>
        ))}
        </div>
    </div>
